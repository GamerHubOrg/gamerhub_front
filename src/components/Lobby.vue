...
const checkIsOwner = () => data.value.users?.some(({
                                                     email,
                                                     isOwner
                                                   }) => email === currentUser.value?.email && !!isOwner);
...
<template>
  <div class="fixed left-0 top-[60px] p-2 w-full max-w-[400px] h-full bg-slate-800 z-10">
