<template>
  <div class="mt-6 flex flex-col gap-6">
    ...
    <div>
            ...
            <transition enter-active-class="transition ease-out duration-100"
              ...
              <MenuItems
                ...
                <div class="py-1">
                  ...
                  <span @click="() => handleSetActiveSort(option)" class="block px-4 py-2 text-sm cursor-pointer"
                    :class="[
      ...
      <div class="bg-dark2">
        <div class="mx-auto max-w-7xl px-6 py-3 flex items-center">
          ...
          <tbody class="divide-y divide-white/5">
            <tr v-for="config in configs" :key="config._id">
              <td class="py-2 pl-0 pr-8 font-semibold table-cell lg:pr-20">
                <div class="flex">
                  <div
                    class="rounded-md bg-gray-700/40 px-2 py-1 text-xs font-medium text-gray-300 ring-1 ring-inset ring-white/10">
                    {{ config.upvotes }}</div>
                </div>
              </td>
              ...
              <td class="hidden py-2 pl-0 pr-8 font-semibold sm:table-cell">
                <User :id="config.userId" v-slot="{ loading, user }">
                  ...
                  <div v-else-if="user" class="flex items-center gap-x-4 text-xs">
                    <img :src="user.picture" alt="" class="h-6 w-6 rounded-full bg-gray-800" />
                    <div class="truncate font-medium leading-6 text-white">
                      <span>{{ user.username }}</span>
                    </div>
                  </div>
                  <div v-else class="truncate font-medium leading-6 text-white text-xs">
                    <span>unknown</span>
                  </div>
                </User>
              </td>
              <td class="hidden py-2 pl-0 pr-4 text-right font-semibold sm:table-cell sm:pr-6 lg:pr-8">
                <div class="text-gray-300 text-xs">{{ new Date(config.createdAt).toLocaleDateString() }}</div>
              </td>
              <td class="py-2 font-semibold table-cell">
    ...
    <nav class="flex items-center justify-between border-t border-primary px-4 sm:px-0">
      ...
      <div class="hidden md:-mt-px md:flex">
        <span v-for="(_, index) in pagination" :key="index"
